...
$(document).ready(function() {
  ...
  recipeList.forEach(initializeRecipeButtons);

  function initializeRecipeButtons(item, index) {
    var recipeWithoutDashes = recipeList[index].recipe.replace(/-/g, " ");
    $("<button class='recipe-buttons' id='recipe-" + index + "'  value='" + index + "'>" +     recipeWithoutDashes + "</button><hr id='hr-recipe-" + index + "'>").appendTo("#recipes");
  }
  
  fetchIngredientsInstructions (selectedRecipeIndex);
  
  function fetchIngredientsInstructions (selectedRecipeIndex) {
    for (var k = 0; k < recipeList[selectedRecipeIndex].ingredients.length; k++) {
      $("<li>" + recipeList[selectedRecipeIndex].ingredients[k] + "</li>").appendTo("#ingredients");
    }
    for (var l = 0; l < recipeList[selectedRecipeIndex].directions.length; l++) {
      $("<li>" + recipeList[selectedRecipeIndex].directions[l] + "  </li>").appendTo("#instructions");
    } 
    $(".recipe-buttons").css("background-color", "#FFFFFF");
    $("#recipe-" + selectedRecipeIndex).css("background-color" ,"#B0BEC5");
  }
  ...
  function updateRecipeButtons () {
    ...
    for (var j=0; j<recipeList.length; j++) {
      var addRecipe = recipeList[j].recipe.replace(/-/g, " ");
      $("<button class='recipe-buttons' id='recipe-" + j + "'  value='" + j + "'>" +     addRecipe + "</button><hr id='hr-recipe-" + j + "'>").appendTo("#recipes");
    }
  }
  
  $("#delete-recipe").click(function() {
    //$("#recipes").text(""); 
    recipeList.splice(selectedRecipeIndex, 1);
    $('#recipe-' + selectedRecipeIndex).remove();
    $("#hr-recipe-" + selectedRecipeIndex).remove();
    $("#ingredients").text("");
    $("#instructions").text("");
    if( recipeList.length > 0 ){
      ...
      fetchIngredientsInstructions (selectedRecipeIndex);
      updateRecipeButtons ();
      $("#recipe-0").trigger('click');
    }
    else if (recipeList.length == 0) {
      selectedRecipeIndex = null;
    }
    else {
      alert ("There are no recipes, please add a new recipe.");  
    }
  });
  
  $("#edit-recipe").click( function () {
    if (selectedRecipeIndex == null) {
      alert ("There are no recipes to edit, please add a recipe");
    }
    else if (recipeList.length > 0) {
      $("#recipe-name-form-edit").text(recipeList[selectedRecipeIndex].recipe); 
      $("#ingredients-form-edit").text(recipeList[selectedRecipeIndex].ingredients);
      $("#instructions-form-edit").text(recipeList[selectedRecipeIndex].directions); 
    }
  });
  
    
  
  $("body").on("click",".recipe-buttons", function() {
    selectedRecipeIndex = this.value;
    $("#ingredients").text("");
    $("#instructions").text("");
    fetchIngredientsInstructions (selectedRecipeIndex);
 });
  
  //Add recipe dialog config
  $(function() {
    $("#dialog").dialog({
    autoOpen: false
    });
  ...
  $("#submit-edit").click(function(e) {
    var recipeName = $("#recipe-name-form-edit").val();
    var ingredients = $("#ingredients-form-edit").val();
    var instructions = $("#instructions-form-edit").val();
    var ingredientsSeparated = ingredients.split(',');
    var instructionsSeparated = instructions.split(',');
    if (recipeName === '' || ingredients === '') {
      ...
      recipeList[selectedRecipeIndex].recipe = recipeName;
      $("#recipe-" + selectedRecipeIndex).html(recipeName);
      recipeList[selectedRecipeIndex].ingredients = ingredientsSeparated;
      recipeList[selectedRecipeIndex].directions = instructionsSeparated;
      $("#ingredients").text("");
      $("#instructions").text("");
      fetchIngredientsInstructions (selectedRecipeIndex);
     }
...
// Validating Form Fields.....
  $("#submit").click(function(e) {
    var recipeName = $("#recipe-name-form").val();
    var ingredients = $("#ingredients-form").val();
    var instructions = $("#instructions-form").val();
    var ingredientsSeparated = ingredients.split(',');
    var instructionsSeparated = instructions.split(',');
    newRecipe["recipe"] = recipeName;
      newRecipe["ingredients"] = ingredientsSeparated;
      newRecipe["directions"] = instructionsSeparated;
      recipeList.push(newRecipe);
      var indexMinusOne = recipeList.length -1;
      ...
      if (recipeName === '' || ingredients === '') {
        ...
        $('<button class="recipe-buttons" id="recipe-' + indexMinusOne + '" value="' + indexMinusOne + '">' + recipeList[indexMinusOne].recipe + "</button><hr id='hr-recipe-" + indexMinusOne + "'>").appendTo("#recipes");
        $("#ingredients").text("");
        $("#instructions").text("");
        selectedRecipeIndex = indexMinusOne;
        fetchIngredientsInstructions(selectedRecipeIndex);  
    }
  $("#dialog").dialog("close");
  return false; 
